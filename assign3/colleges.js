...
function displayResults(resultsArr) {
	...
	for (const element of resultsArr) {
		const row = tbody.insertRow(0);
		var cellIndex = 0;

		const relevantKeys = ["nickname", "SATh", "SATl", "tuition"];
...
searchForm.onsubmit = (e) => {
	...
	const { schoolType, maxTuition, maxSat, minSat } = e.target

	let resultingColleges = univArray;

	if (schoolType.value) {
		resultingColleges = resultingColleges.filter(univ => univ.ownership === schoolType.value)
	}

	if (maxTuition.value) {
		resultingColleges = resultingColleges.filter(univ => univ.tuition <= maxTuition.value)
	}

	if (maxSat.value) {
		resultingColleges = resultingColleges.filter(univ => univ.SATh <= maxSat.value)
	}

	if (minSat.value) {
		resultingColleges = resultingColleges.filter(univ => univ.SATl >= minSat.value)
	}

	displayResults(resultingColleges)
}